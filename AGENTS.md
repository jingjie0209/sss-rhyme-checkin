...
## Durable product direction (2026-09-21)

- This nursery-rhyme check-in prototype is theme-first: the primary screen is a 14-theme accordion, with every theme collapsed until the user opens it.
- Bottom navigation contains only `主题` and `统计`; do not reintroduce separate `资源` and duplicate `主题` tabs.
- Each song has a persistent check-in control. Statistics show total-song completion first, then completion progress for every theme.
- Song details should present content previews (video poster, audio player surface, lyric image, flashcard images) rather than exposing cloud-drive paths as the primary UI.
